...
//       formData.append('bestseller', bestseller ? 'true' : 'false'); // బూలియన్‌ను స్ట్రింగ్‌గా కన్వర్ట్ చేయడం
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setImage4(false);
        setPrice('');
        setBestseller(false); // ఫారమ్ రీసెట్ చేసినప్పుడు bestsellerని రీసెట్ చేయండి
      } else {
        toast.error(response.data.message);
      }
    } catch (error) {
